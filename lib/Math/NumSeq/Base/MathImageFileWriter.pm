...
package Math::NumSeq::Base::MathImageFileWriter;
use 5.006;
use strict;
use warnings;
use File::Temp;
...
sub new {
  ...
  my $fh;
  if (my $filetemp = $Math::NumSeq::Base::MathImageFile::filetemp{$key}) {
    my $filename = $filetemp->filename;
    ### existing file: $filename
    open $fh, '+>', $filename or die;
  } else {
    my $basename = $key;
    $basename =~ s/^Math::NumSeq::MathImage//;
    $basename =~ tr/:/_/;
    $fh = $Math::NumSeq::Base::MathImageFile::filetemp{$key}
