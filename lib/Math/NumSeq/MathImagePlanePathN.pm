...
package Math::NumSeq::MathImagePlanePathN;
use 5.004;
use strict;
use Carp;
...
sub new {
  ...
  $self->{'i_step'} = 1;
  if (my $func = $planepath_object->can("MathImage__NumSeq_${line_type}_step")) {
    $self->{'i_step'} = $planepath_object->$func();
  } elsif ($planepath_object->can("MathImage__NumSeq_A2")
...
{ package Math::PlanePath::PentSpiral;
  use constant MathImage__NumSeq_X_axis_step => 2;
}
{ package Math::PlanePath::TriangleSpiral;
...
=item C<$seq = Math::NumSeq::MathImagePlanePathN-E<gt>new (key=E<gt>value,...)>
...
# Local variables:
# compile-command: "math-image --values=MathImagePlanePathN"
# End:
