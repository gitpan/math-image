...
# math-image --path=MathImageGosperReplicate --lines --scale=10
# math-image --path=MathImageGosperReplicate --all --output=numbers_dash
#

package Math::PlanePath::MathImageGosperReplicate;
use 5.004;
use strict;
...
This path is a self-similar hexagonal tiling of the plane.  At each level
the shape is the Gosper island.
...
The inner N=0 to N=6 are a hexagon, but the individual points can be taken
to represent hexagons to tile the plane,
...
The FlowsnakeCentres uses this same replication shape, but starting from a
side instead of the middle and with rotations and reflections which make it
join up.  Notice for instance N=6 and N=7 here are not adjacent, whereas in
the FlowsnakeCentres they are.  The Flowsnake curve itself is this
replication too, but following edges.

=head2 Complex Base

This path corresponds to expressing complex integers X+i*Y in a base
b=5/2+i*sqrt(3)/2 with a bit of scaling to fit equilateral triangles to a
square grid,

    X/2 + i*Y/(sqrt(3)/2) = a[n]*b^n + ... + a[2]*b^2 + a[1]*b + a[0]

where each digit a[i] is either 0 or a sixth root of unity and is encoded
into N as base 7 integer digits,
...
=item C<$path = Math::PlanePath::MathImageGosperReplicate-E<gt>new ()>
