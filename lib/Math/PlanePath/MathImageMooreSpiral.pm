...
# math-image --path=MathImageMooreSpiral --all --output=numbers_dash
# math-image --path=MathImageMooreSpiral,arms=2 --all --output=numbers_dash
...
package Math::PlanePath::MathImageMooreSpiral;
use 5.004;
use strict;
...
sub xy_to_n {
  ...
  my @x = _digit_split ($x, 3);
  my @y = _digit_split ($y, 3);
  ### @x
  ### @y
...
Math::PlanePath::MathImageMooreSpiral -- 9-segment self-similar spiral

=head1 SYNOPSIS

 use Math::PlanePath::MathImageMooreSpiral;
 my $path = Math::PlanePath::MathImageMooreSpiral->new;
...
=item C<$path = Math::PlanePath::MathImageMooreSpiral-E<gt>new ()>
