...
# math-image --path=MathImagePyramidReplicate --lines --scale=10
# math-image --path=MathImagePyramidReplicate --all --output=numbers_dash --size=80x50

package Math::PlanePath::MathImagePyramidReplicate;
use 5.004;
use strict;
...
sub n_to_xy {
  my ($self, $n) = @_;
  ### MathImagePyramidReplicate n_to_xy(): $n
...
sub xy_to_n {
  my ($self, $x, $y) = @_;
  ### MathImagePyramidReplicate xy_to_n(): "$x, $y"
...
sub rect_to_n_range {
  ...
  ### MathImagePyramidReplicate rect_to_n_range(): "$x1,$y1  $x2,$y2"
...
=item C<$path = Math::PlanePath::MathImagePyramidReplicate-E<gt>new ()>
