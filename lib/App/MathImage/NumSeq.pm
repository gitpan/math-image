...
This is a base class for number sequences.  It's a touch rough yet, but
offers an iterator through the sequence, or random access and predicate for
some sequences.

The idea is to generate things like squares or primes in a generic way.
Some sequences (like squares) are so easy that there's no need for this
oopery except for the genericness, others sequences are trickier and an
iterator is a good way to go through the values.
...
=item C<$seq = App::MathImage::NumSeq::Foo-E<gt>new (key=E<gt>value,...)>
...
Return true if the sequence is of the given C<$key> (a string) type.  This
is intended as a loose set of characteristics or properties a sequence might
have.
...
Return the Online Encyclopedia of Integer Sequences A-number (a string) for
C<$seq>, or C<undef> if not in the OEIS or not known to be.  For example for
the squares the return is "A000290".  The web page for that is then
...
The following methods are only implemented for some sequences, because for
some it can be difficult to generate a particular numbered element etc.
