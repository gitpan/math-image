...
package App::MathImage::Values::Base::FileWriter;
use 5.006;
use strict;
use warnings;
use File::Temp;
...
sub new {
  ...
  my $fh;
  if (my $filetemp = $App::MathImage::Values::Base::File::filetemp{$key}) {
    my $filename = $filetemp->filename;
    ### existing file: $filename
    open $fh, '+>', $filename or die;
  } else {
    my $basename = $key;
    $basename =~ s/^App::MathImage::Values::Sequence:://;
    $basename =~ tr/:/_/;
    $fh = $App::MathImage::Values::Base::File::filetemp{$key}
