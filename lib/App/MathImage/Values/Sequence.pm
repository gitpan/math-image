...
package App::MathImage::Values::Sequence;
use 5.004;
use strict;
...
sub name {
  ...
  $name =~ s/^App::MathImage::Values::Sequence:://;
  return $name;
}
...
App::MathImage::Values::Sequence -- base class for number sequences
...
This is a base class for number sequences.  It's a touch rough yet, but the
idea is iterate through things like squares or primes in a generic way.
...
=item C<$seq = App::MathImage::Values::Sequence::Foo-E<gt>new (key=E<gt>value,...)>
...
have.  The following keys exist currently,
...
Return the Online Encyclopedia or Integer Sequences A-number for C<$seq>, or
C<undef> if C<$seq> is not in the OEIS (so far as is known).  For example
for the sequence of squares the return is "A000290".
