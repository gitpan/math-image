...
package App::MathImage::Values::Sequence::OEIS;
use 5.004;
use strict;
use Carp;
...
sub parameter_list {
  ...
  return  ({
            ...
            type_hint  => 'oeis_anum',
            width      => 8,
            minimum    => App::MathImage::Values::OeisCatalogue->anum_first,
            maximum    => App::MathImage::Values::OeisCatalogue->anum_last,
...
sub new {
  ...
  my $seq_class = $info->{'class'};
  my $parameters_href = $info->{'parameters_href'};
  require Module::Load;
  Module::Load::load($seq_class);
  return $seq_class->new (%options, %{$info->{'parameters_hashref'}});
}
...
App::MathImage::Values::Sequence::OEIS -- OEIS sequence

=head1 SYNOPSIS

 use App::MathImage;
 my $seq = App::MathImage::Values::Sequence::OEIS->new
             (anum => 'A000032');
 my ($i, $value) = $seq->next;
 $value = $seq->ith(6);
...
This module creates a Values for a given OEIS sequence number.  If there's
Values code implementing the sequence then that's used, otherwise downloaded
OEIS files are read.
...
L<App::MathImage::Values::Sequence>
