...
package App::MathImage::Values::Sequence::Expression;
use 5.004;
use strict;
use Carp;
use List::Util;
...
{
  package App::MathImage::Values::Sequence::Expression::LanguageExpr;
...
sub new {
  ...
  if ($evaluator eq 'MS') {
    ...
    my @vars = grep {
      do {
        no strict;
        ! defined ${"App::MathImage::Values::Sequence::Expression::LanguageExpr::$_"}
      }
    } @$varef;
    if (@vars > 1) {
    ...
    $subr = eval "package App::MathImage::Values::Sequence::Expression::LanguageExpr;
                  use strict;
